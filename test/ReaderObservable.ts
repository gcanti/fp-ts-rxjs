...
describe('ReaderObservable', () => {
    describe('Monad', () => {
        it('map', async () => {
            ...
            const x = await pipe(_.of(1), _.map(double))({}).pipe(bufferTime(10)).toPromise()
        ...
        it('chain', async () => {
            ...
            const e1 = await pipe(_.of('foo'), _.chain(f))({}).pipe(bufferTime(10)).toPromise()
            assert.deepStrictEqual(e1, [3])
        })
    })

    it('ask', async () => {
        const e = await _.ask<number>()(1).pipe(bufferTime(10)).toPromise()
    ...
    it('fromOption', async () => {
        const a = await _.fromOption(O.some(1))({}).pipe(bufferTime(10)).toPromise()
    ...
    it('fromReaderTask', async () => {
        const e = await _.fromReaderTask(RT.of(1))({}).pipe(bufferTime(10)).toPromise()
        assert.deepStrictEqual(e, [1])
    })

    it('fromReader', async () => {
        const e = await _.fromReader(R.of(1))({}).pipe(bufferTime(10)).toPromise()
        assert.deepStrictEqual(e, [1])
    })
    ...
    it('zero', async () => {
        const events = await _.zero()({}).pipe(bufferTime(10)).toPromise()
    ...
    it('getMonoid', async () => {
        const M = _.getMonoid()
        const e = await M.concat(_.of('a'), M.empty)({}).pipe(bufferTime(10)).toPromise()
        assert.deepStrictEqual(e, ['a'])
        const e2 = await M.concat(M.empty, _.of('b'))({}).pipe(bufferTime(10)).toPromise()
        assert.deepStrictEqual(e2, ['b'])
        const e3 = await M.concat(_.of('a'), _.of('b'))({}).pipe(bufferTime(10)).toPromise()
    ...
    it('sequence parallel', async () => {
        ...
        const ns = await sequenceParallel([t1, t2])({}).pipe(bufferTime(10)).toPromise()
