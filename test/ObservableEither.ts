...
describe('ObservableEither', () => {
    ...
    it('fromTaskEither', async () => {
        const e = await _.fromTaskEither(TE.right(1)).pipe(bufferTime(10)).toPromise()
        assert.deepStrictEqual(e, [E.right(1)])
    })
    ...
    it('tryCatch', async () => {
        assert.deepStrictEqual(await pipe(rxOf(1), _.tryCatch).pipe(bufferTime(10)).toPromise(), [E.right(1)])
    ...
    it('fold left', async () => {
        ...
        const e = await pipe(_.left(2), _.fold(f, g)).pipe(bufferTime(10)).toPromise()
    ...
    it('fold right', async () => {
        const f = (n: number): Observable<number> => rxOf(n * 2)
        const g = (n: number): Observable<number> => rxOf(n * 3)
        const e = await pipe(_.right(3), _.fold(f, g)).pipe(bufferTime(10)).toPromise()
        assert.deepStrictEqual(e, [9])
    })

    it('getOrElse (left)', async () => {
        const onLeft = (s: string): Observable<number> => rxOf(s.length)
        const e = await pipe(_.left('four'), _.getOrElse(onLeft)).pipe(bufferTime(10)).toPromise()
        assert.deepStrictEqual(e, [4])
    })

    it('getOrElse (right)', async () => {
        const onLeft = (s: string): Observable<number> => rxOf(s.length)
        const e = await pipe(_.right(1), _.getOrElse(onLeft)).pipe(bufferTime(10)).toPromise()
        assert.deepStrictEqual(e, [1])
    })

    it('orElse (left)', async () => {
        const onLeft = (s: string): _.ObservableEither<number, number> => _.left(s.length)
        const e = await pipe(_.left('four'), _.orElse(onLeft)).pipe(bufferTime(10)).toPromise()
        assert.deepStrictEqual(e, [E.left(4)])
    })

    it('orElse (right)', async () => {
        const onLeft = (s: string): _.ObservableEither<number, number> => _.left(s.length)
        const e = await pipe(_.right(1), _.orElse(onLeft)).pipe(bufferTime(10)).toPromise()
        assert.deepStrictEqual(e, [E.right(1)])
    })
    ...
    describe('Monad', () => {
        ...
        it('map', async () => {
            ...
            const x = await pipe(_.right(1), _.map(double)).pipe(bufferTime(10)).toPromise()
        ...
        it('chain', async () => {
            ...
            const e1 = await pipe(_.right('four'), _.chain(f)).pipe(bufferTime(10)).toPromise()
            assert.deepStrictEqual(e1, [E.right(4)])
            const e2 = await pipe(_.right('a'), _.chain(f)).pipe(bufferTime(10)).toPromise()
            assert.deepStrictEqual(e2, [E.left('text')])
            const e3 = await pipe(_.left('b'), _.chain(f)).pipe(bufferTime(10)).toPromise()
        ...
        it('left identity', async () => {
            ...
            const a = 'text'
            const e1 = await pipe(_.of<string, string>(a), _.chain(f)).pipe(bufferTime(10)).toPromise()
    ...
    it('chainFirst', async () => {
        ...
        const e1 = await pipe(_.right('aaaa'), _.chainFirst(f)).pipe(bufferTime(10)).toPromise()
    ...
    describe('Bifunctor', () => {
        it('bimap', async () => {
            ...
            const e1 = await pipe(_.right(1), _.bimap(f, g)).pipe(bufferTime(10)).toPromise()
            assert.deepStrictEqual(e1, [E.right(false)])
            const e2 = await pipe(_.left('foo'), _.bimap(f, g)).pipe(bufferTime(10)).toPromise()
            assert.deepStrictEqual(e2, [E.left(3)])
        })

        it('mapLeft', async () => {
            const double = (n: number): number => n * 2
            const e = await pipe(_.left(1), _.mapLeft(double)).pipe(bufferTime(10)).toPromise()
