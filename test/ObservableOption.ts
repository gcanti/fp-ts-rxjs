...
describe('ObservableOption', () => {
    it('fromIO', async () => {
        const e = await lastValueFrom(_.fromIO(IO.of(1)).pipe(bufferTime(10)))
    ...
    it('tryCatch', async () => {
        assert.deepStrictEqual(await lastValueFrom(pipe(rxOf(1), _.tryCatch).pipe(bufferTime(10))), [O.some(1)])
    ...
    it('fold left', async () => {
        ...
        const e = await lastValueFrom(pipe(_.none, _.fold(f, g)).pipe(bufferTime(10)))
    ...
    it('fold some', async () => {
        const f = (): Observable<number> => rxOf(2)
        const g = (n: number): Observable<number> => rxOf(n * 3)
        const e = await lastValueFrom(pipe(_.some(3), _.fold(f, g)).pipe(bufferTime(10)))
        assert.deepStrictEqual(e, [9])
    })

    it('getOrElse (left)', async () => {
        const onLeft = (): Observable<number> => rxOf(4)
        const e = await lastValueFrom(pipe(_.none, _.getOrElse(onLeft)).pipe(bufferTime(10)))
        assert.deepStrictEqual(e, [4])
    })

    it('getOrElse (some)', async () => {
        const onLeft = (): Observable<number> => rxOf(2)
        const e = await lastValueFrom(pipe(_.some(1), _.getOrElse(onLeft)).pipe(bufferTime(10)))
        assert.deepStrictEqual(e, [1])
    })

    it('alt (left)', async () => {
        const onLeft = (): _.ObservableOption<number> => _.some(4)
        const e = await lastValueFrom(pipe(_.none, _.alt(onLeft)).pipe(bufferTime(10)))
        assert.deepStrictEqual(e, [O.some(4)])
    })

    it('alt (some)', async () => {
        const onLeft = (): _.ObservableOption<number> => _.none
        const e = await lastValueFrom(pipe(_.some(1), _.alt(onLeft)).pipe(bufferTime(10)))
        assert.deepStrictEqual(e, [O.some(1)])
    })
    ...
    describe('Monad', () => {
        ...
        it('map', async () => {
            ...
            const x = await lastValueFrom(pipe(_.some(1), _.map(double)).pipe(bufferTime(10)))
        ...
        it('chain', async () => {
            ...
            const e1 = await lastValueFrom(pipe(_.some('four'), _.chain(f)).pipe(bufferTime(10)))
            assert.deepStrictEqual(e1, [O.some(4)])
            const e2 = await lastValueFrom(pipe(_.some('a'), _.chain(f)).pipe(bufferTime(10)))
            assert.deepStrictEqual(e2, [O.none])
            const e3 = await lastValueFrom(pipe(_.none, _.chain(f)).pipe(bufferTime(10)))
        ...
        it('left identity', async () => {
            ...
            const a = 'text'
            const e1 = await lastValueFrom(pipe(_.of<string>(a), _.chain(f)).pipe(bufferTime(10)))
    ...
    it('apFirst', () => {
        return lastValueFrom(pipe(_.some(1), _.apFirst(_.some(2))).pipe(bufferTime(10))).then(events => {
            assert.deepStrictEqual(events, [O.some(1)])
        })
    })

    it('apFirst', () => {
        return lastValueFrom(pipe(_.some(1), _.apSecond(_.some(2))).pipe(bufferTime(10))).then(events => {
    ...
    it('chainFirst', async () => {
        const f = (a: string): _.ObservableOption<number> => (a.length > 2 ? _.some(a.length) : _.none)
        const e1 = await lastValueFrom(pipe(_.some('aaaa'), _.chainFirst(f)).pipe(bufferTime(10)))
    ...
    it('fromOption', async () => {
        assert.deepStrictEqual(await lastValueFrom(_.fromOption(O.some(1)).pipe(bufferTime(10))), [O.some(1)])
        assert.deepStrictEqual(await lastValueFrom(_.fromOption(O.none).pipe(bufferTime(10))), [O.none])
    })

    it('filterOrElse', async () => {
        assert.deepStrictEqual(
            await lastValueFrom(_.filterOrElse((n: number) => n > 0)(_.of(1)).pipe(bufferTime(10))),
            [O.some(1)]
        )
        assert.deepStrictEqual(
            await lastValueFrom(_.filterOrElse((n: number) => n > 0)(_.of(-1)).pipe(bufferTime(10))),
    ...
    it('filterOrElse', async () => {
        assert.deepStrictEqual(await lastValueFrom(_.fromPredicate((n: number) => n > 0)(1).pipe(bufferTime(10))), [
            O.some(1),
        ])
        assert.deepStrictEqual(await lastValueFrom(_.fromPredicate((n: number) => n > 0)(-1).pipe(bufferTime(10))), [
            O.none,
